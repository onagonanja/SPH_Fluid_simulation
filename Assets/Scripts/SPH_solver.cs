...
public class SPH_solver : MonoBehaviour {
    ...
    HashGridSearcher gridSearcher; // Hash grid searcher for neighbor search

    public float GRAVITY = -0.81f;
    public float BOX_SIZE = 3.0f;

    [SerializeField] float targetDensity = 5.0f; // Target density for the fluid
    // [SerializeField] float pressureEosScale = 7 * 200; // Scale for the equation of state
    // [SerializeField] float pressureEosExponent = 7.0f; // Exponent for the equation of state
    ...
    float PressureEOS(float density) {
        // float p = pressureEosScale / pressureEosExponent * Mathf.Max(Mathf.Pow(density / targetDensity, pressureEosExponent) - 1.0f, 0.0f);
        float p = pressureCoefficient * Mathf.Max(Mathf.Pow(density / targetDensity, 7) - 1.0f, 0.0f);
        // float p = 200 * Mathf.Max(density - targetDensity, 0); // Equation of state for pressure
        return p;
    }
    ...
    void CalcPressureGradient() {
        ...
        Action<int, int> action = (int i, int j) => {
            ...
            particles[i].pressureGradient += particles[i].mass * particles[j].mass * KernelGradient(distance, direction)
        ...
        //     float distance = Vector3.Distance(particles[i].transform.position, particles[j].transform.position);
        //     Vector3 direction = (particles[j].transform.position - particles[i].transform.position).normalized;
        //     particles[i].pressureGradient += particles[j].mass * KernelGradient(distance, direction) * (particles[j].pressure - particles[i].pressure) / (2 * particles[j].density);
        // };
        gridSearcher.forEachNeighbor(particles, action);
    }
    ...
    void CalcViscocity() {
        ...
        Action<int, int> action = (int i, int j) => {
            float distance = Vector3.Distance(particles[i].transform.position, particles[j].transform.position);
            particles[i].viscosity += viscosityCoefficient * particles[i].mass * particles[j].mass * KernelLaplacian(distance) * (particles[j].v - particles[i].v) / particles[j].density;
        };
    ...
    void CalcAcceleration() {
        ...
        for (int i = 0; i < numParticles; i++) {
            ...
            ave_dencity += particles[i].density; // Calculate average density
        }
        ave_dencity /= numParticles; // Average density
    ...
    void CalcWallPenalty() {
        for (int i = 0; i < numParticles; i++) {
            if (particles[i].transform.position.y < -BOX_SIZE) {
                ...
                particles[i].a += new Vector3(0, -dist * WallStiffness, 0); // Wall penalty
            }
            ...
            if (particles[i].transform.position.x > BOX_SIZE) {
                ...
                particles[i].a += new Vector3(-dist * WallStiffness, 0, 0); // Wall penalty
            }
            ...
            if (particles[i].transform.position.z > BOX_SIZE) {
                ...
                particles[i].a += new Vector3(0, 0, -dist * WallStiffness); // Wall penalty
            }
    ...
    void CalcTimeStep() {
        for (int i = 0; i < numParticles; i++) {
            particles[i].v += particles[i].a * Time.deltaTime / 100; // Update velocity
            Vector3 p = particles[i].transform.position;
            if (p.x < BOX_SIZE && p.x > -BOX_SIZE && p.y > -BOX_SIZE && p.z < BOX_SIZE && p.z > -BOX_SIZE) {
                particles[i].v = Vector3.ClampMagnitude(particles[i].v, 5); // Clamp velocity to prevent excessive speed   
            }
            Vector3 newPos = particles[i].transform.position + particles[i].v * Time.deltaTime; // Update position
            particles[i].transform.position = newPos; // Update position
        }
    }
